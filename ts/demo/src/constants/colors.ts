import twColors from "tailwindcss/colors";

const colors = {
  gray: {
    ...twColors.gray,
    150: "#EBEDF0",
  },
  slate: {
    // Neutros beculture (structure): 900 = Slate 900 da marca #0F172B. §4.1.
    50: "#F8FAFC",
    100: "#F1F5F9",
    150: "#E9EEF5",
    200: "#E2E8F0",
    300: "#CBD5E1",
    400: "#94A3B8",
    500: "#64748B",
    600: "#475569",
    700: "#334155",
    800: "#1E293B",
    900: "#0F172B",
    950: "#020617",
  },
  neutral: {
    ...twColors.neutral,
    150: "#EDEDED",
  },
  navy: {
    50: "#E7E9EF",
    100: "#C2C9D6",
    200: "#A5AFC4",
    300: "#6D7EA1",
    400: "#5C6B8A",
    450: "#465675",
    500: "#384766",
    600: "#313E59",
    700: "#24314A",
    750: "#222E45",
    800: "#202B40",
    900: "#182030",
  },
  mirage: {
    50: "#DDE5F5",
    100: "#B4BFD9",
    200: "#9EAAC4",
    300: "#6C7C9E",
    400: "#3D4E70",
    450: "#293859",
    500: "#1E2B47",
    600: "#1A2640",
    700: "#101A2E",
    750: "#0F1729",
    800: "#0C1221",
    900: "#050A16",
  },
  black: {
    50: "#EBEBED",
    100: "#D9D9DE",
    200: "#C5C5CC",
    300: "#93939C",
    400: "#4A4A4F",
    450: "#333338",
    500: "#242428",
    600: "#1F1F21",
    700: "#131314",
    750: "#0C0C0D",
    800: "#080809",
    900: "#000000",
  },
  mint: {
    50: "#E1E5EA",
    100: "#C5CCD3",
    200: "#A0ABB6",
    300: "#70838F",
    400: "#506877",
    450: "#384954",
    500: "#2A3942",
    600: "#242F38",
    700: "#152129",
    750: "#111B22",
    800: "#0D161C",
    900: "#0A1014",
  },
  cinder: {
    // Dark scheme beculture: fundo Slate 900 #0F172B com superfícies navy.
    50: "#E2E8F0",
    100: "#CBD5E1",
    200: "#94A3B8",
    300: "#64748B",
    450: "#3E4C63",
    400: "#4A5A73",
    500: "#334155",
    600: "#273458",
    700: "#1E2A48",
    750: "#1A2540",
    800: "#16203A",
    900: "#0F172B",
  },
  indigo: twColors.indigo,
  blue: twColors.blue,
  green: twColors.green,
  amber: {
    // Amber oficial beculture (primary). 400 = #FFCA28 (assinatura). §4.1/4.2.
    50: "#FFF8E1",
    100: "#FFECB3",
    200: "#FFE082",
    300: "#FFD54F",
    400: "#FFCA28",
    500: "#FFC107",
    600: "#FFB300",
    700: "#FFA000",
    800: "#FF8F00",
    900: "#FF6F00",
    950: "#E65100",
  },
  purple: twColors.purple,
  rose: twColors.rose,
  variants: {
    "secondary-lighter": "#FF75DF",
    "secondary-light": "#FF2ECF",
    secondary: "#E000AD",
    "secondary-darker": "#B8008C",
    "info-lighter": twColors.sky["400"],
    "info-light": twColors.sky["500"],
    info: twColors.sky["600"],
    "info-darker": twColors.sky["700"],
    "success-lighter": "#34D399",
    "success-light": "#10B981",
    success: "#059669",
    "success-darker": "#047857",
    "warning-lighter": "#FFD54F",
    "warning-light": "#FFC107",
    warning: "#FFB300",
    "warning-darker": "#FFA000",
    "error-lighter": "#F87171",
    "error-light": "#EF4444",
    error: "#DC2626",
    "error-darker": "#B91C1C",
  },
};

export { twColors, colors };

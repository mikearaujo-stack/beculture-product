// Barra "Pergunte à sua Memória" — portada do beculture/Confi (a barra de
...
export function PromptBar() {
  ...
  const enviar = async () => {
    ...
    // Busca que toca a Memória faz o grafo "pensar" enquanto a resposta não vem.
    const animaGrafo = modo !== "web";
    if (animaGrafo) marcarBuscaMemoria(true);
    try {
      // Referência (Notas/To-do) só faz sentido no cruzamento com a Memória;
      ...
      limparAnexo();
      flashStatus(
        r.origem === "web" ? "↗ respondido via web" : "◈ respondido pela Memória",
        "ok",
        true,
      );
    } catch (e) {

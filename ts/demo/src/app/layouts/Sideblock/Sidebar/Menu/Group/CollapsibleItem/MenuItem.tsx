...
export function MenuItem({ data }: { data: NavigationTree }) {
  ...
  const body = (isActive: boolean) => (
    <div
      ...
      className="flex min-w-0 items-center justify-between gap-2.5 text-xs tracking-wide"
    >
    ...
    <div className="relative flex">
      {disabled ? (
        <div
          aria-disabled="true"
          className={clsx(
            "dark:text-dark-200 min-w-0 flex-1 rounded-md px-3 py-2 font-medium text-gray-800 outline-hidden",
            DISABLED_MENU_CLASS,
          )}
        >
          {body(false)}

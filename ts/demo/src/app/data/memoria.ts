...
// `listarPastasMemoria`). Vale para TODA opção de gravar no Repositório: o tema
...
export interface MemoryCategory {
  /** Id do tema = nome da pasta no Repositório (ex.: "Reuniões", "Plano LATAM"). */
  id: string;
  label: string;
...
/**
 * Monta a lista de temas do Repositório a partir das pastas do vault: um tema por
...
export function buildMemoryCategories(pastas: string[]): MemoryCategory[] {
  return [...new Set(pastas.map((p) => p.trim()).filter(Boolean))]
    ...
    .map((pasta, i) => ({
      id: pasta,
      label: pasta,
      description: `Repositórios guardados na pasta ${pasta}.`,

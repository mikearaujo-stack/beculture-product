...
// ----------------------------------------------------------------------
// Dock de janelas minimizadas — barra fixa no rodapé com um "chip" por modal
// recolhido. Clicar no chip restaura o modal (com o estado preservado); o X
// fecha de vez.
// ----------------------------------------------------------------------
  ...
  return (
    <div className="pointer-events-none fixed inset-x-0 bottom-0 z-[60] flex justify-center px-4 pb-3">
      <div className="pointer-events-auto flex max-w-full flex-wrap items-center justify-center gap-2">
        {minimized.map(({ id, title, Icon, tint }) => (
          <div
            key={id}
          ...
          >
            ...
            >
              ...
              <span className="max-w-[11rem] truncate text-xs-plus font-medium">{title}</span>
            </button>
            <button
              type="button"
              onClick={() => close(id)}

...
// Rotas (login + funil de cadastro/onboarding) onde as configurações de tema
// (light/dark) NÃO devem ser aplicadas — elas valem só dentro da aplicação.
const NO_THEME_ROUTES = ["/login", "/cadastro", "/onboarding"];
const isNoThemeRoute = (pathname: string) =>
  NO_THEME_ROUTES.some((route) => pathname.startsWith(route));

export function ThemeProvider({ children }: { children: ReactNode }) {
  const isDarkOS = useMediaQuery(COLOR_SCHEME_QUERY);

  // Acompanha a rota atual. O ThemeProvider fica acima do RouterProvider, por
  // isso o pathname é semeado a partir do window e mantido em dia pelo
  // <RouteThemeSync /> (renderizado dentro do router).
  const [activePathname, setActivePathname] = useState(
    () => window.location.pathname,
  );
  const skipTheme = isNoThemeRoute(activePathname);
  ...
  // Nas telas de login/cadastro/onboarding ignoramos o dark mode (ficam light).
  const applyDark = isDark && !skipTheme;
  ...
  useLayoutEffect(() => {
    if (applyDark) _html?.classList.add("dark");

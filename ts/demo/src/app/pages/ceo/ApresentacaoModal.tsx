...
export function ApresentacaoModal({ isOpen, close, onMinimize }: Props) {
  ...
  const gerarArquivo = async () => {
    ...
    if (vazio)
      return setErro(`Adicione ao menos um ${isBook ? "capítulo" : "slide"}.`);
    setStep("arquivo");
    try {
      const nome =
      ...
      if (formato === "pptx") {
        ...
        toast("Apresentação gerada", {
          description: "Aberta em nova aba e baixada (.html).",
        });
      }
      setStep("editor");
    } catch (err) {
  ...
  const prepararMemoria = async () => {
    ...
    const dados =
      formato === "pptx"
        ...
        : new Blob([await gerarHtmlApi(formato, roteiro, design)], {
            type: "text/html",
          });
    return {
      conteudo:
                ...
                {FORMATOS.map((f) => {
                  ...
                  return (
                    ...
                    >
                      <span
                        className={clsx(
                          "text-xs-plus block font-semibold",
                          on
              ...
              edSlides.map((s, i) => (
                ...
                >
                  <div className="mb-2 flex items-center gap-2">
                    ...
                    <IconBtn
                      label="Subir"
                      onClick={() => setEdSlides((a) => move(a, i, -1))}
                    >
                    ...
                    <IconBtn
                      ...
                      danger
                      onClick={() =>
                        setEdSlides((a) => a.filter((_, k) => k !== i))
                      }
                    >
                  ...
                  <MemoriaTextarea
                    ...
                    onChange={(e) =>
                      setEdSlides((a) =>
                        a.map((x, k) =>
                          k === i ? { ...x, bulletsText: e.target.value } : x,
                        ),
                      )
                    }
                    rows={3}
              ...
              edCaps.map((c, i) => (
                ...
                >
                  <div className="mb-2 flex items-center gap-2">
                    ...
                    <IconBtn
                      label="Subir"
                      onClick={() => setEdCaps((a) => move(a, i, -1))}
                    >
                      <ArrowUpIcon className="size-4" />
                    </IconBtn>
                    <IconBtn
                  ...
                  <MemoriaTextarea
                    ...
                    onChange={(e) =>
                      setEdCaps((a) =>
                        a.map((x, k) =>
                          k === i ? { ...x, parasText: e.target.value } : x,
                        ),
                      )
                    }
                    rows={5}
            ...
            <Button
              ...
              onClick={() =>
                isBook
                  ...
                  : setEdSlides((a) => [
                      ...a,
                      { titulo: "", bulletsText: "", notas: "" },
                    ])
              }
              className="gap-2 self-start"
            >
              <PlusIcon className="size-5" /> Adicionar{" "}
              {isBook ? "capítulo" : "slide"}
            </Button>

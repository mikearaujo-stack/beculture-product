...
// a IA gera uma ATA estratégica detalhada e a SALVA na Memória (Reuniões).
// ----------------------------------------------------------------------
...
export function TranscricaoModal({ isOpen, close, onMinimize }: Props) {
  ...
  // Pergunta pós-upload: só abre quando a ata foi de fato salva na Memória.
  ...
  const gerar = async () => {
    setErro("");
    if (!arquivo && !texto.trim()) return setErro("Cole a transcrição ou envie um arquivo.");
    setLoading(true);
    try {
      ...
      setAta(data.ata);
      setSalvo(data.salvo);
      toast(data.salvo ? "Ata salva na Memória" : "Ata gerada", {
        description: data.salvo ? "Guardada em Reuniões." : "Não foi possível salvar na Memória.",
      });
                ...
                {ata !== null ? (
                      ...
                      <div className="min-w-0">
                        ...
                        {salvo && (
                          <p className="flex items-center gap-1 text-xs text-emerald-600 dark:text-emerald-400">
                            <CheckCircleIcon className="size-4" /> Salva na Memória · Reuniões
                          </p>
                        )}
                      </div>
                  ...
                  <div className="grid place-items-center py-10">
                    <div className="flex flex-col items-center gap-3 text-center">
                      ...
                      <p className="dark:text-dark-200 text-sm text-gray-600">Formatando e gerando a ATA estratégica…</p>
                      <p className="dark:text-dark-300 text-xs text-gray-400">Transcrições longas podem levar ~1 minuto.</p>
                    </div>
                  </div>
                ) : (
                  <form onSubmit={(e) => { e.preventDefault(); gerar(); }} className="flex flex-col gap-3">
                    <p className="dark:border-primary-500/20 dark:bg-primary-500/10 rounded-lg border border-primary-200 bg-primary-50 px-3 py-2 text-xs-plus text-gray-600 dark:text-dark-200">
                      Gera uma <b>ATA estratégica e detalhada</b> a partir da transcrição e a <b>salva na Memória</b> (Reuniões), com <span className="font-mono">[[relacionamentos]]</span> às diretrizes existentes.
                    </p>

                    <div>
                      <label className="dark:text-dark-200 mb-1 block text-xs-plus font-medium text-gray-600">Cole a transcrição da reunião</label>
                      <MemoriaTextarea
                        value={texto}
                    ...
                    <div>
                      <label className="dark:text-dark-200 mb-1 block text-xs-plus font-medium text-gray-600">Instruções <span className="text-gray-400">(opcional)</span></label>
                      <MemoriaInput
                        value={instrucoes}

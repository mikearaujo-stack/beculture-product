...
function buildCarrosselHtml(
  ...
  const cardsHtml = cards
    .map((c, i) => {
      const eyebrow =
        i === 0 ? "CAPA" : i === total - 1 ? "CTA" : `${i + 1} / ${total}`;
      return (
...
export function CarrosselModal({ isOpen, close, onMinimize }: Props) {
  ...
  const copiarLegenda = async () => {
    ...
    try {
      ...
      toast("Copiado", {
        description: "Legenda + hashtags na área de transferência.",
      });
    } catch {
  ...
  const baixarHtml = () => {
    ...
    const nome =
      c.titulo.replace(/[^\p{L}\p{N}\-_ ]/gu, "").slice(0, 60) || "carrossel";
    ...
    toast("Carrossel gerado", {
      description: "Aberto em nova aba e baixado (.html).",
    });
  };
  ...
  const prepararMemoria = async () => {
    ...
    const corpo = c.cards
      ...
      .join("\n\n");
    const rodape = [c.legenda, c.hashtags.map((h) => `#${h}`).join(" ")]
      .filter(Boolean)
      .join("\n\n");
    return {
      conteudo: [corpo, rodape, c.conexoes.trim()]
        .filter(Boolean)
        .join("\n\n---\n\n"),
      anexos: [
        {
          nome: "carrossel.html",
          dados: new Blob(
            [buildCarrosselHtml(c.titulo, c.cards, c.legenda, c.hashtags)],
            {
              type: "text/html",
            },
          ),
          ...
          <div className="grid place-items-center py-10">
            <div className="flex flex-col items-center gap-3 text-center">
              ...
              <p className="dark:text-dark-200 text-sm text-gray-600">
                A IA está montando o carrossel…
              </p>
            </div>
          </div>
        )}
          ...
          <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
            ...
            <div className="flex flex-col gap-3">
              <input
                ...
                className="form-input dark:border-dark-500 dark:bg-dark-800 dark:text-dark-50 w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm font-semibold placeholder:text-gray-400"
              />

              {cards.map((c, i) => (
                <div
                  key={i}
                  className="dark:border-dark-600 dark:bg-dark-800/40 rounded-xl border border-gray-200 p-3"
                >
                  <div className="mb-2 flex items-center gap-2">
                    <span className="dark:bg-dark-600 dark:text-dark-200 grid size-6 shrink-0 place-items-center rounded-md bg-gray-100 text-xs font-semibold text-gray-500">
                      {i + 1}
                    </span>
                    <input
                      value={c.titulo}
                    ...
                    <IconBtn label="Descer" onClick={() => move(i, 1)}>
                  ...
                  <MemoriaTextarea
                    ...
                    className="form-textarea dark:border-dark-500 dark:bg-dark-800 dark:text-dark-100 text-xs-plus w-full resize-y rounded-lg border border-gray-300 bg-white px-2.5 py-1.5 placeholder:text-gray-400"
                  />
                </div>
              ))}
              <Button
              ...
              <div>
                ...
                <MemoriaTextarea
                  ...
                  rows={3}
                  className="form-textarea dark:border-dark-500 dark:bg-dark-800 dark:text-dark-100 w-full resize-y rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm"
                />
              </div>
              <div>
                <label className="dark:text-dark-200 text-xs-plus mb-1 block font-medium text-gray-600">
                  ...
                  <span className="text-gray-400">
                    (separadas por espaço, sem #)
                  </span>
                </label>
                <input
                  value={hashtags}
                  onChange={(e) => setHashtags(e.target.value)}
                  className="form-input dark:border-dark-500 dark:bg-dark-800 dark:text-dark-100 w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm"
                />
              </div>
            </div>
            ...
            {/* Coluna prévia + ações */}
            <div className="flex flex-col gap-3">
              <div className="border-dark-600 relative aspect-square w-full overflow-hidden rounded-2xl border bg-[#111827] p-8">
                <div className="absolute inset-y-0 left-0 w-2 bg-[#FFCA28]" />
                <div className="absolute top-6 left-8 text-xs font-bold tracking-widest text-[#FFCA28]">
                  {idx === 0
                    ? "CAPA"
                    : idx === cards.length - 1
                      ? "CTA"
                ...
                <div className="flex h-full flex-col justify-center">
                  <h3 className="text-2xl leading-tight font-extrabold text-[#F8FAFC]">
                    {card?.titulo || "—"}
                  </h3>
                  {card?.texto && (
                    <p className="mt-3 text-sm leading-relaxed text-[#E2E8F0]">
                      {card.texto}
                    </p>
                  )}
                </div>
                <span className="absolute right-5 bottom-4 text-xs text-[#94A3B8]">
              ...
              <div className="flex items-center justify-center gap-2">
                ...
                <span className="dark:text-dark-300 min-w-14 text-center text-xs text-gray-400">
                  {cards.length ? idx + 1 : 0} / {cards.length}
                </span>
                <IconBtn
                  label="Próximo"
                  onClick={() =>
                    setIdx((n) => Math.min(cards.length - 1, n + 1))
                  }
                >
              ...
              <div className="dark:border-dark-600 dark:bg-dark-800/40 rounded-xl border border-gray-200 p-3">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    disabled={refinando}
                    placeholder="Ex.: mais provocativo, menos texto…"

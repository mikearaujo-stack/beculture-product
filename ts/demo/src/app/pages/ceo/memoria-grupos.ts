// ----------------------------------------------------------------------
// Pasta da Memória de cada grupo (agrupamento) — fonte única de verdade.
//
...
/** Pasta do grupo dentro da Pasta da Memória (mesma limpeza dos nomes de nota). */
...
/**
 * Feedback padrão de falha ao gravar na Pasta da Memória. Navegador sem suporte
...
export function avisarFalhaMemoria(reason: FalhaMemoria) {
  ...
  if (reason === "no-folder") {
    toast("Nenhuma pasta da Memória definida", {
      description:
        "Defina a pasta em Memória (ou Configurações) para o grupo ter pasta própria.",
    });
    return;
  }
  if (reason === "denied") {
  ...
  toast("Não foi possível gravar na pasta da Memória", {

...
// um RESUMO (terminando com as conexões Obsidian), salvo no Repositório (Reuniões).
// ----------------------------------------------------------------------
...
export function AudioModal({ isOpen, close, onMinimize }: Props) {
  ...
  // Pergunta pós-upload: só abre quando o resumo foi de fato salvo no Repositório.
  ...
  const gerar = async () => {
    ...
    try {
      ...
      toast(data.salvo ? "Resumo salvo no Repositório" : "Resumo gerado", {
        description: data.salvo ? "Guardado em Reuniões." : "Não foi possível salvar no Repositório.",
      });
                ...
                {resumo !== null ? (
                      ...
                      <div className="min-w-0">
                        ...
                        {salvo && (
                          <p className="flex items-center gap-1 text-xs text-emerald-600 dark:text-emerald-400">
                            <CheckCircleIcon className="size-4" /> Salvo no Repositório · Reuniões
                          </p>
                        )}
                      </div>
                  ...
                  <form onSubmit={(e) => { e.preventDefault(); gerar(); }} className="flex flex-col gap-3">
                    <p className="dark:border-primary-500/20 dark:bg-primary-500/10 rounded-lg border border-primary-200 bg-primary-50 px-3 py-2 text-xs-plus text-gray-600 dark:text-dark-200">
                      A IA <b>transcreve o áudio</b> (OpenAI Whisper) e gera um <b>resumo</b> (com as conexões Obsidian ao final), salvo no Repositório (Reuniões).
                    </p>

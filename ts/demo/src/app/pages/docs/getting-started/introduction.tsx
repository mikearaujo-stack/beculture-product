export function Introduction() {
  return (
    <section>
      <h3
        id="introduction"
        className="dark:border-dark-500 mt-10 scroll-mt-20 border-b border-gray-200 pb-1 text-lg font-semibold lg:text-2xl"
        data-heading="Introduction"
        data-order="2"
      >
        <a href="#introduction">Introduction</a>
      </h3>
      <div className="text-sm-plus mt-5 space-y-3">
        <p>
          beculture is a cutting-edge React-based dashboard theme designed for
          developers seeking efficiency, flexibility, and aesthetics. Built with
          Tailwind CSS, it offers a seamless blend of Dark & Light Layouts,
          ensuring a visually engaging user experience.
        </p>
        <p>
          With its multi-color schemes, monochrome mode, and ultra-responsive
          layouts, beculture adapts effortlessly to any project. Its modular
          architecture and lightweight dependencies promise fast performance and
          easy scalability.
        </p>
        <p>
          beculture comes loaded with powerful tools like React Hook Forms,
          TanStack Table, and Headless UI, paired with pre-built apps (Chat,
          Mail, Kanban, and more) and pages (Help, Auth, Errors, etc.) for
          faster development. Its creative widgets, advanced tables, and
          beautiful components make crafting interfaces a breeze.
        </p>
        <p>
          Developers will appreciate the easy customization, Yarn build tools,
          and well-documented structure, while users enjoy stunning dashboards
          for diverse needs: Sales, Analytics, Crypto, LMS, and more.
        </p>
        <p>
          beculture combines functionality, performance, and design to deliver a
          theme that elevates your projects to the next level.
        </p>
      </div>
    </section>
  );
}

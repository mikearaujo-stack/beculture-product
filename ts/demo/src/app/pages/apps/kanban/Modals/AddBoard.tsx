...
const schema = Yup.object({
  name: Yup.string().required("Informe o nome do quadro"),
  columns: Yup.array().of(
    Yup.object().shape({
      name: Yup.string().required("Informe o nome da etapa"),
      color: Yup.string().required("Selecione uma cor").oneOf(colors),
    }),
  ),
  isPrivate: Yup.bool(),
});
...
function AddBoardForm({ close }: { close: () => void }) {
  ...
  const {
    ...
    defaultValues: {
      name: "",
      columns: [
        { name: "A fazer", color: "info" },
        { name: "Em andamento", color: "warning" },
        { name: "Concluído", color: "success" },
      ],
      isPrivate: false,
    },
  });
  ...
  return (
    <form className="pt-4" onSubmit={handleSubmit(onSubmit)} autoComplete="off">
      ...
      <div className="mt-4">
        <Switch label="Quadro privado" {...register("isPrivate")} />
      </div>
